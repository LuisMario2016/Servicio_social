...
c3ann <- read.csv("C:/serviciosocial/proporciones/c3ann.csv")
c4ann <- read.csv("C:/serviciosocial/proporciones/c4ann.csv")
c3per <- read.csv("C:/serviciosocial/proporciones/c3per.csv")
c4per <- read.csv("C:/serviciosocial/proporciones/c4per.csv")
c3nfx <- read.csv("C:/serviciosocial/proporciones/c3nfx.csv")
pastr <- read.csv("C:/serviciosocial/proporciones/pastr.csv")
#secmb <- read.csv("C:/serviciosocial/proporciones/secmb.csv")
#secma <- read.csv("C:/serviciosocial/proporciones/secma.csv")
lista<-list(primn,primf,range,urban,secdn,c3ann,c4ann,
            c3per,c4per,c3nfx,pastr)
library(dplyr)
...
nuevos_nombres <- c("primn", "primf", "range","urban", "secdn",
                    "c3ann","c4ann","c3per","c4per","c3nfx","pastr")
row.names(Mexico) <- nuevos_nombres
Mexico<-as.data.frame(Mexico)
Mexico<-Mexico[,-1]
Mexico<-as.data.frame(Mexico)
años <- c(1895, 1910, 1925, 1940, 1955, 1970, 1985, 2000, 2015)
Mexico <- Mexico %>%
  mutate(años = años) %>%
  select(años, everything())

ruta<- "C:\\serviciosocial"
write.csv(Mexico, file.path(ruta, "Mexico.csv"), row.names = FALSE) ##

plotmex <- read.csv("C:/serviciosocial/Mexico.csv")

plot(plotmex$X, Mexico$primn, ylim = c(0,0.7), type = "l", col = "#00A600")
points(plotmex$X, plotmex$primf, col = "#2DB600", type = "l")
points(plotmex$X, plotmex$range, col = "#63C600", type = "l")
points(plotmex$X, plotmex$urban, col = "#A0D600", type = "l")
points(plotmex$X, plotmex$secdf, col = "#E6E600", type = "l")
points(plotmex$X, plotmex$c3ann, col = "#E8C727", type = "l")
points(plotmex$X, plotmex$c4ann, col = "#EAB64E", type = "l")
points(plotmex$X, plotmex$c3per, col = "#ECB176", type = "l")
points(plotmex$X, plotmex$c4per, col = "#EEB99F", type = "l")
points(plotmex$X, plotmex$c3nfx, col = "#F0CFC8", type = "l")
points(plotmex$X, plotmex$pastr, col = "#F2F2F2", type = "l")
